...
import {
  ...
  MessageSquare, // for templates
  MessageCircle, // contact messages icon
} from "lucide-react";
...
function MenuItems({ onNavigate }) {
  // get role from redux store (adjust selector if your store shape is different)
  const { user } = useSelector((state) => state.auth || {});
  const role = user && user.role ? String(user.role).toLowerCase() : null;

  // admin gets only products, orders, reviews
